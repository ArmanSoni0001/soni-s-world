...
st.set_page_config(page_title="Sentiment Analysis for Amazon Product", layout="wide")

def fetch_data():
    link=st.text_input("paste product review link")
    url=link
    if not url:
        st.stop()
    if url:   
...
def read_data():
    ...
    if uploaded_file:
        ...
        df_c['polarity']= df_c['review_list'].apply(pol)
        fig = px.scatter(df_c, 
                            x='polarity', 
                            y='pos', 
                            color = 'Sentiment',
...
def main():
    ...
    if st.button('Get Link From browser'):
